...
const Orders = ({url}) => {
  ...
  const statusHandler = async (event,orderId)=>{
    ...
    const response = await axios.post(url+"/api/order/status",{orderId,status:event.
    target.value})
    if(response.data.success){
    ...
    <section className="p-4 sm:p-10 box-border w-full">
      ...
      <div className="overflow-auto mt-5">
        <table className="w-full">
          ...
          <tbody>
            {orders.map((order,i)=>(
              <tr key={i} className="border-b border-slate-900/20 text-gray-50 
              p-6 medium-14 text-left">
                ...
                <td className="p-1">
                  <select onChange={(event)=>statusHandler(event,order._id)} value={order.status} className="bg-primary ring-1 ring-secondary text-sm max-w-20 xl:max-w-28">
                    <option value="Product Loading">Product 
                    Loading</option>
